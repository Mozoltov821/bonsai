# Copyright 2025 Bonsai Team.
"""JAX implementation of MiMo-Audio interface with sharding support."""
...
class MimoAudio:
    """JAX implementation of MimoAudio interface with sharding support"""
    ...
    def __init__(
        ...
        Args:
            ...
            mesh: Optional JAX mesh for sharding. If None and use_sharding=True, creates default mesh
            use_sharding: Whether to enable sharding (default: True)
        """
        ...
        # Setup sharding
        if use_sharding and mesh is None:
            devices = jax.devices()
            num_devices = len(devices)
            if num_devices >= 2:
                # Use tensor parallelism only (fsdp=1)
                mesh_shape = (1, num_devices)
                devices_reshaped = np.array(devices).reshape(mesh_shape)
                mesh = jax.sharding.Mesh(devices_reshaped, ('fsdp', 'tp'))
                print(f"Created default mesh: fsdp=1, tp={num_devices}")
            else:
                print("Single device detected, sharding disabled")
                use_sharding = False
        ...
        # Create config with sharding if enabled
        if use_sharding:
            config = MiMoAudioConfig.with_sharding(
                **{k: v for k, v in config_dict.items() if k in MiMoAudioConfig.__dataclass_fields__}
            )
            print("✅ Sharding enabled for main model")
        else:
        ...
        self.model = create_model_with_weights(
            ...
            args=args,
            rngs=nnx.Rngs(0),
            mesh=mesh if use_sharding else None,
        )
        ...
        # Enable sharding for tokenizer if requested
        if use_sharding:
            tokenizer_config_dict['use_sharding'] = True
        ...
        self.mimo_audio_tokenizer = load_tokenizer_weights_from_safetensors(
            ...
            dtype=jnp.float32,  # Tokenizer uses float32
            mesh=mesh if use_sharding else None,
            rngs=nnx.Rngs(0)
        )
        ...
        if use_sharding:
            print("✅ Sharding enabled for audio tokenizer")
        ...
        # Task-specific samplers (matching official implementation)
    ...
    def encode_batch(self, input_features: jnp.ndarray, input_lens: jnp.ndarray):
        ...
        if total_length <= max_length:
            ...
            for length in input_lens:
                chunk = input_features[current_pos:current_pos+int(length), :]
                codes_output = self.mimo_audio_tokenizer.encode(
                    chunk[jnp.newaxis, :, :],
                    jnp.array([length]),
                    use_quantizer=True
