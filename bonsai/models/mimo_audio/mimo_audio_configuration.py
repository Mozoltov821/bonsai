...
class MiMoAudioConfig:
    """Configuration for MiMo Audio Model (使用实际模型的固定值)"""
    ...
    def create_qwen2_config(self) -> "Qwen2Config":
        """Create Qwen2 config for main transformer (固定配置)"""
        from bonsai.models.qwen2.modeling import ModelConfig as Qwen2Config
    ...
    def create_local_qwen2_config(self) -> "Qwen2Config":
        """Create local transformer Qwen2 config (固定配置)"""
        from bonsai.models.qwen2.modeling import ModelConfig as Qwen2Config
    ...
    def create_input_local_qwen2_config(self) -> "Qwen2Config":
        """Create input local transformer Qwen2 config (固定配置)"""
        from bonsai.models.qwen2.modeling import ModelConfig as Qwen2Config
        ...
        return Qwen2Config(
            ...
            norm_eps=1e-6,
            tie_word_embeddings=False,
            use_causal_mask=False,  # input_full_attention=True
            shd_cfg=self.shd_cfg,
        )
