"""
MiMo Audio 模型层级一致性验证脚本 (FP32 版本)

该脚本用于验证 PyTorch 版本和 JAX 版本的 MiMo Audio 模型的逐层输入输出一致性。
使用 FP32 精度以获得更高的数值准确性。

与 bfloat16 版本的主要区别:
1. 所有模型计算使用 float32 精度
2. 由于显存限制，分两步执行：先运行 JAX，保存激活值后卸载，再运行 PyTorch
3. 更高的精度可以更清楚地暴露实现差异

主要功能:
1. 加载 JAX 模型 (fp32)，运行前向传播，保存激活值，卸载模型
2. 加载 PyTorch 模型 (fp32)，运行前向传播，保存激活值
3. 对比两组激活值的数值差异
4. 生成详细的对比报告

使用方法:
    python bonsai/models/mimo_audio/test/test_layer_comparison_fp32.py
"""
...
class ComparisonConfig:
    """对比配置"""
    # 容差设置 (FP32 精度，比 bfloat16 更严格)
    rtol: float = 1e-5  # 相对容差 0.001%
...
class JAXLayerCapture:
    ...
    def forward_with_capture(
        ...
        if self.config.compare_input_local_transformer:
            ...
            speech_embeds_flat = speech_embeds.reshape(B_orig * T_groups, group_size, hidden_size).astype(jnp.float32)
        ...
        # 10. Local transformer (如果需要，使用 FP32)
...
class LayerComparisonRunner:
    ...
    def run(self) -> Dict:
        """执行完整的层级对比 (FP32 版本 - 分步执行以节省显存)"""
        print("=" * 70)
        print("MiMo Audio 层级一致性验证 (FP32)")
        print("=" * 70)
        print("\n⚠️  注意：由于显存限制，将分两步执行：")
        print("  1. 先运行 JAX 模型，保存激活值后卸载")
        print("  2. 再运行 PyTorch 模型，然后对比结果")
        print()
        ...
        # 4. 保存 JAX 激活值到内存（转换为 NumPy 以释放 JAX 内存）
        print("\n[4/9] 保存 JAX 激活值并释放模型...")
        jax_activations_numpy = {}
        for name, data in jax_activations.items():
            jax_activations_numpy[name] = {
        ...
        print("✅ JAX 激活值已保存，模型已卸载")

        # ===== 第二阶段：PyTorch 模型 =====
        print("\n" + "=" * 70)
        print("第二阶段：PyTorch 模型")
        print("=" * 70)
        ...
        for layer_name in sorted(jax_activations_numpy.keys()):
            if layer_name not in torch_activations:
                print(f"⚠ {layer_name}: PyTorch 中缺失")
                continue

            jax_act = jax_activations_numpy[layer_name]['value']
        ...
        # 9. 保存结果
        print("\n[9/9] 生成报告...")
        self._save_results(report, jax_activations_numpy, torch_activations)
